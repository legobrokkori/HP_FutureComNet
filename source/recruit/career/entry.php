...
if ($_SERVER["REQUEST_METHOD"] === "POST"){
	...
	if ($_POST['entryCategory'] === ""){
		$_POST['entryCategory'] = "2";
	}

	$_POST['entryMessagge'] = "中途採用";
	...
	# 各画面毎の振る舞い　基底クラス
	$_POST['entryDivision'] = new Career();
...
<meta name="description" content="FCN | FutureComnet | ヒューチャーコムネットの中途採用エントリーページです。">
		...
		<div id="breadCrumbsArea">
			<div class="inner">
				<ul id="breadCrumbs">
					...
					<li><strong>キャリア採用｜応募フォーム</strong></li>
				</ul>
			</div>
		</div>
		<div id="rectuitContentsArticleArea" role="main">
			<div class="inner">
				<h1 id="contentsArticle">キャリア採用<br><span id="en">career</span></h1>
			</div>
			<div id="backGroundPictureCar">
			</div>
		</div>
		<div class="inner">
			<div id="mainAreaEntryForm" role="main">
				<div class="jobContents">
					<h2 class="enrtyHeader">キャリア採用応募フォーム</h2>
							...
							<dl>
								...
								<dd>
									<dl>
										...
										<dd>
											...
											<input type="checkbox" class="validate[minCheckbox[1]" name="occupation" value="システムエンジニア">
											<label>システムエンジニア</label><br>
											<input type="checkbox" class="validate[minCheckbox[1]" name="occupation" value="Webデザイナー">
											<label>Wed デザイナー</label><br>
										</dd>
										<dt>営業職</dt>
										<dd>
								...
								<dd>
									<p style="margin: 10px 0 10px 0">
										...
										<select name="skillExcell">
											<?php $selSkillExcell = isset($_POST['skillExcell']) ? $_POST['skillExcell'] : ''; ?>
											<option value="" selected>--選択してください--</option><option value="未経験" <?php if($selSkillExcell === "未経験") echo "selected"; ?>>未経験</option><option value="初級" <?php if($selSkillExcell === "初級") echo "selected"; ?>>初級</option><option value="中級" <?php if($selSkillExcell === "中級") echo "selected"; ?>>中級</option><option value="上級" <?php if($selSkillExcell === "上級") echo "selected"; ?>>上級</option>
										</select>
									</p>
									<p style="margin: 10px 0 10px 0">
										<label>Word</label><br>
										<select name="skillWord">
											<?php $selSkillWord = isset($_POST['skillWord']) ? $_POST['skillWord'] : ''; ?>
											<option value="" selected>--選択してください--</option><option value="未経験" <?php if($selSkillWord === "未経験") echo "selected"; ?>>未経験</option><option value="初級" <?php if($selSkillWord === "初級") echo "selected"; ?>>初級</option><option value="中級" <?php if($selSkillWord === "中級") echo "selected"; ?>>中級</option><option value="上級" <?php if($selSkillWord === "上級") echo "selected"; ?>>上級</option>
										</select>
									</p>
									<p style="margin: 10px 0 10px 0">
										<label>PowerPoint</label><br>
										<select name="skillPowerPoint">
											<?php $selSkillPowerPoint = isset($_POST['skillPowerPoint']) ? $_POST['skillPowerPoint'] : ''; ?>
											<option value="" selected>--選択してください--</option><option value="未経験" <?php if($selSkillPowerPoint === "未経験") echo "selected"; ?>>未経験</option><option value="初級" <?php if($selSkillPowerPoint === "初級") echo "selected"; ?>>初級</option><option value="中級" <?php if($selSkillPowerPoint === "中級") echo "selected"; ?>>中級</option><option value="上級" <?php if($selSkillPowerPoint === "上級") echo "selected"; ?>>上級</option>
										</select>
									</p>
									<p style="margin: 10px 0 10px 0">
										<label>Visio</label><br>
										<select name="skillVisio">
											<?php $selSkillVisio = isset($_POST['skillVisio']) ? $_POST['skillVisio'] : ''; ?>
											<option value="" selected>--選択してください--</option><option value="未経験" <?php if($selSkillVisio === "未経験") echo "selected"; ?>>未経験</option><option value="初級" <?php if($selSkillVisio === "初級") echo "selected"; ?>>初級</option><option value="中級" <?php if($selSkillVisio === "中級") echo "selected"; ?>>中級</option><option value="上級" <?php if($selSkillVisio === "上級") echo "selected"; ?>>上級</option>
										</select>
									</p>
									<p style="margin: 10px 0 10px 0">
										<textarea id="notes" style="width: 550px" rows=3 name="appealPoint" placeholder="スキルについてアピールポイントがある場合、ご記入ください。" value="<?php echo htmlentities($_POST['appealPoint'], ENT_QUOTES,'UTF-8'); ?>"></textarea>
									</p>
								</dd>
								<dt>備考</dt>
								<dd>
